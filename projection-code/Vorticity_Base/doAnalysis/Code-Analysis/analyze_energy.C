...
TH1D* Get_positive_hist(TH1D* h1){

   TH1D* new_h1 = (TH1D*) h1->Clone("new_h1");
   new_h1->Reset("ICES");

   Int_t nbins = h1->GetXaxis()->GetNbins();
      
      for(int jx = 0; jx < nbins; jx++){

      Double_t bin_content = h1->GetBinContent(jx + 1);
      Double_t bin_error = h1->GetBinError(jx + 1);

      if(bin_content >=0){
         new_h1->SetBinContent(jx+1, bin_content);
         new_h1->SetBinError(jx+1, bin_error);
      }

      else{
         new_h1->SetBinContent(jx+1, 0.0);
      }
...
void Write_positive_on_file(TFile* f, Int_t n_entries_tau_cuts){

   //Declaration of variables
   Int_t nbins;
   TH1D* hists_1d[n_entries_tau_cuts];
   TH1D* hists_positive[n_entries_tau_cuts];
   Double_t* Vector_of_taus = new Double_t[n_entries_tau_cuts];


   for(int i = 0; i < n_entries_tau_cuts; i++){
      hists_1d[i] = (TH1D*) f->Get(Form("BulletOnly/Energy/Proj_1D/Energy_tau_%.2f", (i+1) / 4.0));
      Vector_of_taus[i] = (i+1) / 4.0;

      hists_positive[i] = (TH1D*) hists_1d[i]->Clone("hists_positive[i]");
      hists_positive[i]->Reset("ICES");

      hists_positive[i] = Get_positive_hist(hists_1d[i]);

      hists_positive[i]->SetName(Form("Energy_vs_x_tau_%.2f",(i+1) / 4.0 ));

      hists_positive[i]->SetDirectory(gDirectory);
      hists_positive[i]->Write();
...
/*
      ...
      nbins = hists_1d[i]->GetNbinsX();
      Double_t bin_content;
      Double_t bin_error;
      for(int l = 0; l < nbins; l++){

         bin_content = hists_1d[i]->GetBinContent(l+1);
         bin_error = hists_1d[i]->GetBinError(l+1);

         if(bin_content >= 0){
            hists_positive[i]->SetBinContent(l+1, bin_content);
            hists_positive[i]->SetBinError(l+1, bin_error);
         }
         else{
            hists_positive[i]->SetBinContent(l+1, 0.0);   
         }
      }
      */
   ...
   for(int index = 0; index < n_entries_tau_cuts; index++){
      Bin_of_peak[index]          = hists_positive[index]->GetMaximumBin();

      X_of_peak[index]            = hists_positive[index]->GetBinCenter(Bin_of_peak[index]);
      X_of_peak_error[index]      = hists_positive[index]->GetBinError(Bin_of_peak[index]);

      Height_of_peak[index]       = hists_positive[index]->GetBinContent(Bin_of_peak[index]);
      Height_of_peak_error[index] = hists_positive[index]->GetBinError(Bin_of_peak[index]);

      Integral_peak[index]        = hists_positive[index]->Integral(1.0,nbins);

      RMS_of_peak[index]          = hists_positive[index]->GetRMS();
   }
...
void analyze_energy(TString Outname){
//----------------------------------------------------------------------------------------------------/
...
//Open File
   ...
   TFile* f_in_0p08 = TFile::Open("/home/joao/jbarbon/projects/Vorticity/Vorticity3D_Dev/Projections_Chun_Code/bullet_no_bulk_R0p5/Code-Proj/NoBulk_R0p5.root");
   //TFile* f_in_0p01 = TFile::Open("/home/joao/jbarbon/projects/Vorticity/Vorticity3D_Dev/Plots/Projections_Chun_Code/Bullets-data5/Results/Run_Energy/HB_R-.5_EtaOverS-0.01_Proj_Energy.root");
   //TFile* f_in_0p001 = TFile::Open("/home/joao/jbarbon/projects/Vorticity/Vorticity3D_Dev/Plots/Projections_Chun_Code/Bullets-data5/Results/Run_Energy/HB_R-.5_EtaOverS-0.001_Proj_Energy.root");

//----------------------------------------------------------------------------------------------------/
//Declare and initialize histograms
   Int_t Number_Entries_tau_cuts = 23;
...
//TGraphs for area vs tau
   ...
   TG_area_vs_tau_0p08->SetTitle("Energy Peak - area vs tau");
...
//TGraphs for RMS vs tau
   ...
   TG_RMS_vs_tau_0p08->SetTitle("Energy Peak - RMS vs tau");
...
//Multigraph Draw
   ...
   TCanvas* c_area_tau = new TCanvas("Graph area_peak vs tau", "Graph area_peak vs tau",800,600);
   c_area_tau->SetGrid();
   ...
   MG_area_vs_tau->SetTitle("Energy Peak - area vs tau");
   MG_area_vs_tau->GetXaxis()->SetTitle("#tau");
   MG_area_vs_tau->GetYaxis()->SetTitle("area (fm*GeV)");
   MG_area_vs_tau->Draw("a");
   //MG_area_vs_tau->GetXaxis()->SetRangeUser(0.0,14.1);
   legend->Draw("same");


   TCanvas* c_height_tau = new TCanvas("Graph height_peak vs tau", "Graph height_peak vs tau",800,600);
   ...
   MG_height_vs_tau->SetTitle("Height of Energy Peak - height vs tau");
   ...
   MG_RMS_vs_tau->SetTitle("RMS of Energy Peak - RMS vs tau");
   ...
   TCanvas* c_x_tau = new TCanvas("Graph x_peak vs tau", "Graph x_peak vs tau",800,600);
   c_x_tau->SetGrid();
   ...
   MG_x_vs_tau->SetTitle("X position of Energy Peak - x vs tau");
   ...
   TDirectory *Dir_Positive_Hists = f_out->mkdir("PositiveHists");
   f_out->cd();
   gDirectory->cd("PositiveHists");
   Write_positive_on_file(f_in_0p08, 23);
 ...
 */
   ...
   gSystem->Exec("mkdir Proj_Energy");
   c_x_tau->Print("./Proj_Energy/X_vs_tau.png");
   c_RMS_tau->Print("./Proj_Energy/RMS_vs_tau.png");
   c_height_tau->Print("./Proj_Energy/height_vs_tau.png");
   c_area_tau->Print("./Proj_Energy/area_vs_tau.png");
