...
tuple <Double_t*, Double_t*,Double_t*, Double_t*, Double_t*, Double_t* , Double_t*>  Analyze(TFile* f, Int_t n_entries_tau_cuts){
   ...
   for(int i = 0; i < n_entries_tau_cuts; i++){
      hists_1d[i] = (TH1D*) f->Get(Form("BulletOnly/Vort/Proj_1D_y/Vort_tau_%.2f", (i+1) / 4.0));
      Vector_of_taus[i] = (i+1) / 4.0;

      hists_positive[i] = (TH1D*) hists_1d[i]->Clone();
...
//Multigraph Draw
   ...
   TCanvas* c_x_tau = new TCanvas("Graph Y of Vort Peak vs tau", "Graph Y of Vort Peak vs tau",800,600);
   c_x_tau->SetGrid();
   ...
   MG_x_vs_tau->SetTitle("Y position of Vort Peak - Y vs tau");
   MG_x_vs_tau->GetXaxis()->SetTitle("#tau");
   MG_x_vs_tau->GetYaxis()->SetTitle("Y (fm)");
   MG_x_vs_tau->Draw("a");
   ...
   gSystem->Exec("mkdir ProjY_Vort");
   c_x_tau->Print("./ProjY_Vort/Y_vs_tau.png");
   c_RMS_tau->Print("./ProjY_Vort/RMS_vs_tau.png");
   c_height_tau->Print("./ProjY_Vort/height_vs_tau.png");
   c_area_tau->Print("./ProjY_Vort/area_vs_tau.png");
